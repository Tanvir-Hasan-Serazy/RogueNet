...
Tracks online presence, typing indicators, and live delivery for comments and notifications so the product feels truly real-time.

## ADDED Requirements

### Requirement: Online status via presence service
The system SHALL track online status via WebSocket connections with a heartbeat and TTL, and SHALL expose online/offline state to other users.

#### Scenario: User appears online on connect
- **WHEN** a user establishes an authenticated WebSocket connection
...
The system SHALL broadcast ephemeral typing start/stop events within a conversation or post comment context to relevant participants, with automatic timeout, and SHALL NOT persist them.

#### Scenario: Typing start broadcast
- **WHEN** participant A starts typing in conversation C
...
The system SHALL deliver new replies to a post in real time to viewers subscribed to that post's room, in addition to persisting them.

#### Scenario: Live comment delivery
- **WHEN** user A creates a reply on post P while viewer B is subscribed to post P's room
...
The system SHALL deliver notifications via the same WebSocket multiplexed connection used for presence and messaging, with at least once delivery semantics for online users.

#### Scenario: Live notification over existing socket
- **WHEN** a notification is generated for an online user with an active socket
