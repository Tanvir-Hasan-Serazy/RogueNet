...
Implements upvote and downvote per user per post with persisted vote state and a demotion-based ranking signal for the feed.

## ADDED Requirements

### Requirement: Upvote and downvote with single vote per user per post
The system SHALL allow an authenticated user to cast exactly one vote per post with value +1 (upvote) or -1 (downvote), and SHALL allow changing or removing the vote.

#### Scenario: Upvote post
- **WHEN** a user upvotes a post they can view and have not voted on before
...
- **WHEN** a viewer fetches a post or feed that includes a post they voted on
...
The system SHALL maintain accurate up count, down count, and derived score, and SHALL enforce uniqueness of (user_id, post_id) at the storage layer.
