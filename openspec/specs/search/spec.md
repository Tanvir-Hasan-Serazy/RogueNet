...
The system SHALL provide a search endpoint that accepts a query and a mode (`posts` or `people`) and returns results of the selected type, available to authenticated and anonymous viewers (with visibility filtering for posts).

#### Scenario: Search posts mode
- **WHEN** a viewer searches with mode posts and query "hello"
- **THEN** the system returns posts whose body matches the query, respecting visibility for the viewer (using `session.user.id` if authenticated), ordered by relevance then recency

#### Scenario: Search people mode
- **WHEN** a viewer searches with mode people and query "ali"
- **THEN** the system returns better-auth users whose username or display name matches the prefix, ordered by relevance
...
- **WHEN** a user (via better-auth session) creates a post containing the term "roguenet"
